 ...
 * https://github.com/stumped2/school/blob/master/CS411/project3/kernel/drivers/block/ramdisk.c
 * -2-6-31/
 */
...
static void printing(unsigned char *buffer, unsigned int length){
  while((length = length - 1) != 0){
    printk("%02x", *buffer++); //unsigned char type printing
  }
}
